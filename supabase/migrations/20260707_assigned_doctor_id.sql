-- Doktor ataması: assigned_doctor_id kolonu + RLS/RPC hizalaması (koç/diyetisyen ile aynı desen)

alter table public.members add column if not exists assigned_doctor_id uuid;

-- Mevcut JSONB atamalarını kolona taşı
update public.members
set assigned_doctor_id = nullif(data->>'assignedDoctorId', '')::uuid
where assigned_doctor_id is null
  and nullif(data->>'assignedDoctorId', '') is not null;
...
-- Dolu slotlar: doktor kolonu + JSONB yedek
...
-- book_staff_session: doktor staff_id önce kolondan
...
begin
  ...
  if v_type = 'coach' then
    ...
    if v_staff_id is null then v_staff_id := nullif(v_data->>'assignedDoctorId', '')::uuid; end if;
  ...
  where (
    ...
    or (v_type = 'doctor'
      and (m.assigned_doctor_id = v_staff_id or m.data->>'assignedDoctorId' = v_staff_id::text))
  )
  and coalesce(s->>'status', 'scheduled') in ('scheduled', 'rescheduled')
  and (s->>'date') is not null
  and (s->>'date') ~ '^\d{4}-\d{2}-\d{2}T'
