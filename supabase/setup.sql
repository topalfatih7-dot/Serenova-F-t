...
--    healthAnalysis  — GPT-5.4 staff skor + staffBrief (üyeye gösterilmez; program/diyet yok)
...
-- Personel hakediş IBAN / banka (kamuya açık staff.data içinde tutulmaz)
...
create policy staff_payout_accounts_select
  on public.staff_payout_accounts for select to authenticated
  using (staff_id = (select public.current_staff_id()) or public.is_admin());
...
create policy staff_payout_accounts_insert
  on public.staff_payout_accounts for insert to authenticated
...
create policy staff_payout_accounts_update
  on public.staff_payout_accounts for update to authenticated
...
('spor', 'Spor Paketi', 2499, 'Aylık', true, null,
 ...
 '[{"months":1,"label":"Aylık","price":2499},{"months":3,"label":"3 Aylık","price":6499},{"months":6,"label":"6 Aylık","price":9999}]'::jsonb,
 'blue', 3),
('doktor', 'Doktor Paketi', 1500, 'Tek Seferlik', true, 'Tek Seferlik',
 '[{"text":"1 Online Doktor Görüşmesi","included":true},{"text":"Görüntülü Görüşme","included":true},{"text":"Mevcut üyeliğe ek paket olarak eklenebilir","included":true}]'::jsonb,
