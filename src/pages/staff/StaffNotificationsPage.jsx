...
export default function StaffNotificationsPage() {
  ...
  const handleNotificationOpen = (n) => {
    ...
    if (n.type === 'chat' && n.memberId) {
      navigate(`/staff/messages/${n.memberId}`)
    }
  }
  ...
  return (
    <PanelPageShell onPointerDown={() => { unlockNotificationAudio().catch(() => {}) }}>
      <PanelPageHeader
        title="Bildirimler"
        subtitle={unread > 0 ? `${unread} okunmamış bildiriminiz var` : 'Her şey güncel görünüyor'}
        icon={Bell}
        accent="brand"
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={Bell} title="Bildirim yok" description="Yeni randevular ve güncellemeler burada görünecek." />
      ) : (
