...
export function calculatePackagePrice(config) {
  ...
  const subtotal = BASE_PRICE + coachCost + dietitianCost + addOnCost
  const discount = durationWeeks >= 32 ? 0.2 : durationWeeks >= 24 ? 0.15 : durationWeeks >= 16 ? 0.1 : 0
  ...
  return {
    ...
    total,
    monthly: Math.round(total / Math.max(weeks, 1)),
  }
}
