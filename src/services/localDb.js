...
// Açılışta gösterilecek varsayılan kadro (admin panelinden düzenlenebilir/silinebilir)
const DEFAULT_STAFF = [
  { role: 'coach', name: 'Elif Kaya', email: 'elif.kaya@serenova.fit', password: 'Serenova2026!', phone: '0532 000 00 01', specialty: 'Güç & Fonksiyonel Antrenman', bio: 'Sertifikalı kişisel antrenör. Ev ve salon programlarıyla sürdürülebilir form kazandırır.', photo: '/team/team-coach-1.png', workDays: [1, 3, 5], workStart: '09:00', workEnd: '17:00' },
  { role: 'coach', name: 'Mert Demir', email: 'mert.demir@serenova.fit', password: 'Serenova2026!', phone: '0532 000 00 02', specialty: 'Kondisyon & Kilo Yönetimi', bio: 'Kardiyo ve kuvvet dengesiyle, her seviyeye uygun motive edici antrenman planları.', photo: '/team/team-coach-2.png', workDays: [2, 4, 6], workStart: '10:00', workEnd: '18:00' },
  { role: 'dietitian', name: 'Dr. Zeynep Arslan', email: 'zeynep.arslan@serenova.fit', password: 'Serenova2026!', phone: '0532 000 00 03', specialty: 'Klinik Beslenme', bio: 'Kişiye özel, dengeli beslenme programlarıyla kalıcı sağlıklı alışkanlıklar oluşturur.', photo: '/team/team-dietitian-1.png', workDays: [1, 4], workStart: '09:00', workEnd: '16:00' },
  { role: 'dietitian', name: 'Can Yıldız', email: 'can.yildiz@serenova.fit', password: 'Serenova2026!', phone: '0532 000 00 04', specialty: 'Sporcu Beslenmesi', bio: 'Performans ve toparlanma odaklı beslenme stratejileriyle hedeflerinize destek olur.', photo: '/team/team-dietitian-2.png', workDays: [3, 5], workStart: '11:00', workEnd: '19:00' },
]

function seedDefaultStaff(db) {
  if (db.staffSeeded) return false
  db.staffSeeded = true
  DEFAULT_STAFF.forEach((s) => {
    const email = s.email.toLowerCase()
    if (db.staff.some((x) => x.email === email) || db.members.some((m) => m.email === email)) return
    db.staff.push({
      id: uid('st'),
      role: s.role,
      name: s.name,
...
export function initSession() {
  ...
  if (seedDefaultStaff(db)) changed = true
